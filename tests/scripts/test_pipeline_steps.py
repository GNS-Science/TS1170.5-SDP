...
def test_hf_filepath():
    assert str(pipeline_steps.hf_filepath(site_limit=10)) == str(
        Path(WORKING_FOLDER) / "first_20_hcurves.hdf5"
    )
    assert str(pipeline_steps.hf_filepath()) == str(
        Path(WORKING_FOLDER) / "all_hcurves.hdf5"
    )
