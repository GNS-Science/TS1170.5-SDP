...
def ensure_resource_folders(version_id: str, exist_ok: bool = False):
    ...
    pipeline_folder = Path(RESOURCES_FOLDER).parent / "resources" / "pipeline" / f"v{version_id}"
...
class VersionManager:
    ...
    def __init__(self, resource_folder: str | None = None):
    ...
    def read_version_list(self) -> dict[str, VersionInfo]:
        ...
        if not self._version_list_path.exists():
            raise RuntimeError(f"the version_list file {self._version_list_path} was not found.")
